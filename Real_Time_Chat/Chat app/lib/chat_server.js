var socketio = require('socket.io')
  ...
  , namesUsed = []
  , currentRoom = {}
  , avaliableRooms = ['General','Assistance Service'];

function assignGuestName(socket, guestNumber, nickNames, namesUsed) {
  // Generate new guest name
  var name = 'Guest' + guestNumber;

  // Associate guest name with client connection ID Let user know their
  // guest name
...
//handle message in the chatrooms socket server will be waiting for
//message and callback will take care when received.
...
function handleRoomJoining(socket) {
  socket.on('join', function(room) {
	...
	// "~" same as using != -1
	if(~avaliableRooms.indexOf(room.newRoom)){
...
exports.listen = function(server) {
  // Start the Socket.io server, allowing it to piggyback on 
  ...
  io.sockets.on('connection', function (socket) {
	var initialRoom = avaliableRooms[0];
    socket.join(initialRoom);

    // Place user in the "Lobby" room when they connect
    currentRoom[socket.id] = initialRoom;
    socket.emit('joinResult', {room: initialRoom });
    ...
    socket.on('rooms', function() {
      socket.emit('rooms', avaliableRooms);
    });
